...
async function syncSelectedAccountsToKnowledgeBase() {
  if (!checkLogin()) return;

  const rows = getSelectedRows();
  if (!rows.length) return;

  isKbSyncing.value = true;
  ...
  try {
    ...
    const newCount = results.reduce((total, item) => total + item.newCount, 0);
    toast.success('知识库同步完成', `已处理 ${results.length} 个公众号，新增 ${newCount} 篇文章`);
  } catch (e: any) {
    toast.error('知识库同步失败', e?.message || '请求失败');
  } finally {
    isKbSyncing.value = false;
  }
}
    ...
    <div class="flex flex-col h-full divide-y divide-gray-200">
      ...
      <header class="flex items-stretch gap-3 px-3 py-3">
        ...
        <UButton
          ...
          @click="syncSelectedAccountsToKnowledgeBase"
          >知识库同步</UButton
        >
